...
- リリースビルドのバイナリサイズを `-trimpath -ldflags="-s -w"` で削減する
...
- `-trimpath`: バイナリからソースファイルの絶対パスを除去
- `-s -w`: シンボルテーブルと DWARF デバッグ情報を除去してバイナリサイズを削減
...
| `.github/skills/create-github-release/release.ps1` | `go build` に `-trimpath -ldflags "-s -w -X main.version=$Version"` 追加 |
