...
config.json               ← APIキー・設定（gitignore 対象、バイナリ隣に配置）
```
...
viper で `config.json` を読み込む。`APIKey` / `TeamID` フィールド（mapstructure タグ: `apiKey` / `teamId`）。  
...
`config.json` はバイナリと同じディレクトリに配置する（`go run` では `os.Executable` の解決先が異なるため注意）。
